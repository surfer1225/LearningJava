...
public class ExclusiveTimeFunction {
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        Iterator<String> it = logs.listIterator();
        String[] cur = it.next().split(":");
        st.push(Integer.parseInt(cur[0]));
        int timeOffset = Integer.parseInt(cur[2]);

        while (it.hasNext()) {
            cur = it.next().split(":");

            if (cur[1].equals("start")) {
                int newTime = Integer.parseInt(cur[2]);
                if (!st.isEmpty()) res[st.peek()]+=(newTime - timeOffset);
                st.push(Integer.parseInt(cur[0]));
                timeOffset = newTime;
            }
            else {
                int newTime = Integer.parseInt(cur[2]);
                res[st.peek()]+=(newTime - timeOffset + 1);
                timeOffset = newTime + 1;
                st.pop();
            }
        }
    ...
    public static void main(String[] args) {
        ExclusiveTimeFunction ett = new ExclusiveTimeFunction();
        int[] res = ett.exclusiveTime(2,
                new LinkedList<>(Arrays.asList("0:start:0","1:start:2","1:end:5","0:end:6")));
